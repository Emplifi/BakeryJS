...
function qTrace(statsd: boolean = false): MethodDecorator {
	return function (target: any, property: string | symbol, descriptor: PropertyDescriptor): void {
		...
		descriptor.value = function (...argsList: any[]) {
			...
			if (src && tgt) {
				eventEmitter.emit('sent', Date.now(), (this as any).source, (this as any).target, batchSize)
			}
...
function sampleStats<T extends { new (...args: any[]): AQueue<any> }>(constr: T): T {
