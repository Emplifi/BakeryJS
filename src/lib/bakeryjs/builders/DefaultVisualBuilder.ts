...
import {ConcurrentSchemaComponent, SchemaObject, SerialSchemaComponent} from '../IFlowBuilder';
...
export class DefaultVisualBuilder implements IVisualBuilder{
    ...
    private printSerial(series: SerialSchemaComponent, indent: string = ''): string {
        let output = '';
        output += `${indent}SERIAL ${'-'.repeat(TERMINAL_WIDTH - 7 - indent.length)}\n`;
    ...
    private printConcurrent(concurrencies: ConcurrentSchemaComponent, indent: string = ''): string {
        let output = '';
        output += `${indent}CONCURRENT ${'-'.repeat(TERMINAL_WIDTH - 11 - indent.length)}\n`;
